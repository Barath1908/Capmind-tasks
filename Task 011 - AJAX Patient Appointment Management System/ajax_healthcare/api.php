...
// Special action param (e.g. ?action=csrf_token, ?action=doctors)
$action = $_GET['action'] ?? '';

// ── GET CSRF Token ────────────────────────────────────────────
if ($method === 'GET' && $action === 'csrf_token') {
    respond(true, 'Token generated.', ['token' => generateCsrfToken()]);
}

// ── GET Doctors list ─────────────────────────────────────────
if ($method === 'GET' && $action === 'doctors') {
    $result = $conn->query("SELECT id, name, specialty FROM doctors ORDER BY name");
    if (!$result) respond(false, 'Failed to fetch doctors.', null, 500);
    $doctors = [];
    while ($row = $result->fetch_assoc()) $doctors[] = $row;
    respond(true, 'Doctors fetched.', $doctors);
}
...
if ($method === 'GET') {
    ...
    while ($row = $result->fetch_assoc()) {
        unset($row['csrf_token']); // never expose tokens
        $appointments[] = $row;
    }
...
if ($method === 'POST') {
    ...
    // CSRF validation
    $csrfToken = $_SERVER['HTTP_X_CSRF_TOKEN'] ?? ($d['csrf_token'] ?? '');
    ...
    // Fetch the new record to return full object
    $fetch = $conn->prepare(
...
if ($method === 'PUT') {
    ...
    // Double-booking check (exclude current record)
    $chk = $conn->prepare(
