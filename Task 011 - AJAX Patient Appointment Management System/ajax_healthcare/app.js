...
async function fetchCsrfToken() {
    try {
        const res  = await fetch(`${API}?action=csrf_token`);
        const json = await res.json();
        if (json.success) csrfToken = json.data.token;
    } catch (e) {
...
async function loadDoctors() {
    try {
        const res  = await fetch(`${API}?action=doctors`);
        const json = await res.json();
