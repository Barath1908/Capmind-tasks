...
// ID always comes via POST (either from list.php button or the edit form itself)
$id = (int)($_POST['id'] ?? $_SESSION['edit_id'] ?? 0);
if (!$id) { $_SESSION['error']='Invalid ID.'; header('Location:'.BASE_URL.'patients/list.php'); exit; }

// First load from list.php — save id to session then redirect to GET
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['_action'])) {
    $_SESSION['edit_id'] = $id;
    header('Location:' . BASE_URL . 'patients/edit.php');
    exit;
}

// GET request — read id from session
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    $id = (int)($_SESSION['edit_id'] ?? 0);
    if (!$id) { $_SESSION['error']='Invalid ID.'; header('Location:'.BASE_URL.'patients/list.php'); exit; }
}
...
$errors = [];
// '_action' hidden field tells us if it's the EDIT FORM submit or just the first load
$isUpdate = isset($_POST['_action']) && $_POST['_action'] === 'update';
...
if ($isUpdate) {
    ...
    if (!$errors) {
        ...
        if ($s->execute()) {
            $_SESSION['success']='Patient "'.$old['patient_name'].'" updated!';
            header('Location:'.BASE_URL.'patients/list.php'); exit;
        }
        $errors['db'] = 'DB error: '.$conn->error;
    }
    // Validation failed — $old has submitted values, re-render form with errors
} else {
    // First load — pre-fill with DB values
    $old = $patient;
}
...
<div class="page-header">
    <div>
        ...
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active">Edit #<?= $id ?></li>
        </ol>
    </div>
    <span class="text-muted" style="font-size:.8rem">
        <i class="bi bi-clock me-1"></i>Registered: <?= date('d M Y',strtotime($patient['created_at'])) ?>
    </span>
</div>
        ...
        <form method="POST" action="edit.php" novalidate>
            ...
            <div class="form-grid">
                ...
                <div class="field-group">
                    ...
                    <input type="<?= $t ?>" name="<?= $fk ?>"
                           class="form-control <?= $err?'is-invalid':'' ?>"
                           value="<?= htmlspecialchars($old[$fk]) ?>"
                           placeholder="<?= $ph ?>">
                ...
                <div class="field-group">
                    ...
                    <input type="number" name="age"
                           ...
                           placeholder="Years" min="1" max="130">
